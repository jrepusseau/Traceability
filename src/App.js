...
export default class App extends React.Component
{
  ...
  /* getTokenList = () => {
    ...
    buildSendADATransaction = async () => {
        ...
        var iunit = a.options[a.selectedIndex].value

        let b = document.getElementById("vehicle")
        var ivehicle = b.options[b.selectedIndex].value

        let c = document.getElementById("object")
        var iname = c.options[c.selectedIndex].value
        var iid = c.options[c.selectedIndex].key
        var iitemTx = c.options[c.selectedIndex].title


       let f = document.getElementById("receiver")
       var ireceiver = this.encodeUser(f.value);

       let g = document.getElementById("quantity")
       var iqt = g.value

       let h = document.getElementById("distance")
       var idistance = h.value

          let m = document.getElementById("Text1")
       var imes = m.value
...
ty.forEach(function(number) {
    ...
    buildSendADATransactionB = async () => {
        let a = document.getElementById("unitC")
        var iunit = a.options[a.selectedIndex].value

        let b = document.getElementById("batchCid")
        var ibatch = b.value

        let c = document.getElementById("product")
        var iitemTx = c.options[c.selectedIndex].title
        var iid = c.options[c.selectedIndex].key
        var iitemName = c.options[c.selectedIndex].value

       let g = document.getElementById("quantityC")
       var iqt = g.value

       let h = document.getElementById("startDate")
       var istartdate = h.value

       let x = document.getElementById("expDate")
       var iexpdate = x.value




       let y = document.getElementById("nameC")
       var iname = y.value

          let m = document.getElementById("noteC")
       var imes = m.value
...
ty.forEach(function(number) {
        ...
        map.insert(
          ...
          var meta = {
           ...
           // "itemName": iitemName, 
            ...
            "quantity":iqt,
            "unit":iunit,
          "startingdate":istartdate,
            "expirationdate":iexpdate,
            "note":ty
            }
    ...
    buildSendADATransactionC = async () => {
     

        let c = document.getElementById("productP")
        var iitemName = c.value


       let h = document.getElementById("imageP")
       var iimage = h.value

       let x = document.getElementById("idP")
       var iid = x.value
...
console.log(ty)
   ...
   xmp.open("GET", "https://cardano-mainnet.blockfrost.io/api/v0/metadata/txs/labels/24", false); // false for synchronous request
   ...
   console.log(content2.length);
        ...
        {
            console.log(story);
            switch (story.json_metadata.operation) {
                ...
                case 'transfer':
                     ...
                     obj =
                        {
                          tx_hash : story.tx_hash,
                          blocktime : story.blocktime,
                          sender: story.user, 
                          receiver : story.json_metadata.transfer[0].receiver, 
                          itemTx:story.json_metadata.itemTx,
                          quantity : parseInt(story.json_metadata.transfer[0].quantity),
                          unit:story.json_metadata.transfer.unit,
                          itemName: story.json_metadata.itemName, 
                          batchTx: story.json_metadata.batchTx, 
                          name: story.json_metadata.name, 
                          verifiedTx:"", 
                        } 
                           var yourwallet = stakeTest;
         var pos = batchList.map(val => val.tx_hash).indexOf(obj.batchTx)
                        if (yourwallet == story.json_metadata.transfer[0].receiver)
                        {
                            batchList[pos].quantity = obj.quantity;
                  ...
                  case 'reception':
                     obj =
                        ...
                        blocktime : story.blocktime,
                        user : story.user,
                        transferTx: story.json_metadata.transferTx,
                        note: story.json_metadata.note
                        } 
                        console.log('BR');
                    var pos = operations.map(val => val.tx_hash).indexOf(obj.transferTx)
                    console.log(pos)
                    console.log(operations[pos].receiver)
                    console.log(obj.user)
                    if(operations[pos].receiver==obj.user)
                    operations[pos].verifiedTx = obj.tx_hash;
                    console.log( operations[pos]);
                 break;
                 case 'account':
                    obj =
                       {
                       image : story.json_metadata.profile.image,
                       name : story.json_metadata.profile.name,
                       country : story.json_metadata.profile.country,
                       city : story.json_metadata.profile.city,
                       description : story.json_metadata.profile.description
              ...
              { 
                ...
                obj =
               {
                 ...
                 name : story.json_metadata.company,
                 website: story.json_metadata.website,
               ...
               if (story.json_metadata.object == "product")
               { obj =
                {
                    tx: story.tx_hash,
                    address: a.inputs[0].address,
                ...
                if (story.json_metadata.object == "batch")
                { obj =
                 {
                   tx_hash : story.tx_hash,
                   name : story.json_metadata.name,
                   itemTx:story.json_metadata.itemTx,
                   id : story.json_metadata.id,
                   quantity :parseInt(story.json_metadata.quantity),
                   unit:story.json_metadata.unit,
                   startingDate: story.json_metadata.startingDate, 
                   startingDate: story.json_metadata.expirationDate
                 } 
                 batchList.push(obj); 
                 console.log('BC');}
...

{
    ...
    <img className='profile' src={prd.image}/> 
   <h2>{prd.name}</h2> 
...
Company: <input id="name" type="text"></input> <br></br>
...
<hr style={{marginTop: "40px", marginBottom: "40px"}}/>
                    ...
                    <Tab id="3" title="Inventory" panel={
                        <div style={{marginLeft: "20px"}}>
                    <hr style={{marginTop: "20px", marginBottom: "20px"}}/>
                        ...
                        <option value={prd.itemTx}> {prd.name} </option>
                            ))
                        }
                        </select>
                        <a onClick={(e) => this.change("1")}>+ New product</a>

{
                            productList.filter(prd => prd.itemTx == prodChoice).map(prd => (
                            <div>
                    ...
                    {
productList.filter(prd => prd.itemTx == prodChoice).length!=0 &&
<div>
                        ...
                        { batchList.filter(bth => bth.itemTx == prodChoice).map(bth => (
                                <option value={bth.tx_hash}> {bth.name} </option>
                                    ))
                                }</select> <a onClick={(e) => this.change("2")}>+ New batch</a>

                        <ul>
                        {batchList.filter(bth => bth.tx_hash == batchChoice).map(bth => (
                                <li>
                        <p>{bth.name}</p> 
                        <p>{bth.id}</p> 
                        <p>{bth.quantity}</p> 
                        ...
                        {
                            batchList.filter(bth => bth.tx_hash == batchChoice).length!=0 &&
                         <a onClick={(e) => this.change("3")}>Transfer</a>
                        }
                        </div> 
                        }
...

                            {
                                opChoice == 1 &&
                                <div className="tweet">  
                               <a onClick={(e) => this.change("0")}>Back</a>
       
                                <p className="tweet-user"> Create Product </p> 
    Product ID: 
    ...
    <br></br> 
    <button style={{padding: "10px"}} onClick={this.buildSendADATransactionC}>Run</button>
    </div> 
                            }
                            {
                                  opChoice == 2 &&
                           ...
                           <a onClick={(e) => this.change("0")}>Back</a>
                            ...
                            <p className="tweet-user"> Create Batch </p> 
 Select product of the batch: 
                            <select id="product"> 
                            {productList.map((item,index) => (
          <option title={item.itemTx} key={item.id}>{item.name}</option> 
                           ))}
                           </select>
                           <br></br>
 Batch ID: 
...
Name: 
 <input id="nameC" type="text"></input> <br></br>
 Quantity: 
 <input id="quantityC" type="number"></input> <br></br>
   Unit:
                            <select id="unitC"> 
                    <option value="kg">kg</option>
                    </select>  <br></br>
      Start date:
      <input id= "startDate" type="date"></input> <br></br>
      Expiration date:
                            <input id="expDate" type="date"></input> 
                      
                            <br></br>
                Note: 
<textarea id="noteC" cols="40" rows="5"></textarea>
<br></br> 
<button style={{padding: "10px"}} onClick={this.buildSendADATransactionB}>Run</button>
                            ...
                            }
                           {
                            ...
                            <p className="tweet-header"> Send Batch </p> 
                            Receiver: 
                            <input list="user" id="receiver"></input>
                            <datalist id="user">
                                {User.map(us => 
                                        (
                                        <option key={us.user}>{us.username}</option>
                                        )
          ...
          <option title={item.itemTx} key={item.id}>{item.name}</option> 
                            ...
                            <br></br>
                              Quantity: 
                            <input id="quantity" type="number" max={objSelected.quantity}></input> 
                            <br></br>
                            Unit:
                            <select id="unit"> 
                    <option value="kg">kg</option> 
                ...
                {/*
                   ...
                   <input type="text" id="anylabel"></input>:
                            <input type="text" id="anytext"></input> */}
                Note: 
...

                       {
                         operations.map((tw =>                                  
                             <div key={tw} className="tweet">         
                              ...
                              {
                              tw.receiver == stakeTest &&
                                  <div>  
                                    "Import"
                                <span className="tweet-created-on">
                                    {new Date(tw.blocktime*1000).toLocaleString('en-US')}
                                 </span>{' '}·{' '} 
                                     
                               <div className="tweet-content"> Received {tw.quantity}kg
                               {//batchList[batchList.map(val => val.tx_hash).indexOf(tw.batchTx)].name
                               }
                                 {this.decodeUser(tw.receiver)}
                               </div>
                               </div>
                            }
                            {
                              ...
                              <div className="tweet-content"> Send {tw.quantity}kg{
                              //batchList[batchList.map(val => val.tx_hash).indexOf(tw.batchTx)].name
                            } to {this.decodeUser(tw.receiver)}
                              </div>
                              {
                                tw.verifiedTx &&
